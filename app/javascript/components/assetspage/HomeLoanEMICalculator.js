            ...
            try {
                ...
                for (let month = 1; month <= loandetails.tenor_months; month++) {
                    ...
                    calculations.push({
                        ...
                        monthlyInstalment: formatCurrency(emi),
                        openingBalance: formatCurrency(openingBalance),
                        principal: formatCurrency(principal),
                        interest: formatCurrency(interest),
                        outstandingLoanBalance: formatCurrency(outstandingBalance)
                    });
                    ...
                    if (monthofyear === 12) {
                        ...
                        if (year < 6) {
                            summaryCalculations.push({
                                year: year.toString(),
                                totalPrincipal: formatCurrency(summaryYearPrincipal),
                                totalInterest: formatCurrency(summaryYearInterest),
                                totalEMI: formatCurrency(summaryYearEMI)
                            });
                            summaryYearEMI = 0;
