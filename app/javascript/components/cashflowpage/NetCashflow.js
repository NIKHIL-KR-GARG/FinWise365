...
const NetCashflow = ({ netCashflowData }) => {
    ...
    const chartData = () => {
        const yearlyData = netCashflowData.filter(data => data.month === 12).reduce((acc, curr) => {
            ...
            if (!acc[year]) {
                acc[year] = { income: 0, expense: 0, net_position: 0, unlocked_assets: 0, locked_assets: 0, net_worth: 0, age: curr.age };
            }
        ...
        return Object.keys(yearlyData).map(year => ({
            ...
            unlocked_assets: yearlyData[year].unlocked_assets.toFixed(2),
    ...
    const columns = [
        ...
        ...['income', 'expense', 'net_position', 'unlocked_assets', 'locked_assets', 'net_worth'].map(type => ({
            field: type,
    ...
    const rows = chartData().map((row, index) => ({
        ...
        unlocked_assets: row.unlocked_assets,
                    ...
                    <Grid item size={12} display="flex" justifyContent="center" style={{ height: '40vh' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData()}>
                                ...
                                <Line type="monotone" dataKey="income" stroke="#1f77b4" strokeWidth={3} activeDot={{ r: 8 }} />
                                <Line type="monotone" dataKey="expense" stroke="#ff7f0e" strokeWidth={3} activeDot={{ r: 8 }} />
                                <Line type="monotone" dataKey="net_position" stroke="#2ca02c" strokeWidth={3} activeDot={{ r: 8 }} />
                                <Line type="monotone" dataKey="unlocked_assets" stroke="#d62728" strokeWidth={3} activeDot={{ r: 8 }} />
                                <Line type="monotone" dataKey="locked_assets" stroke="#9467bd" strokeWidth={3} activeDot={{ r: 8 }} />
